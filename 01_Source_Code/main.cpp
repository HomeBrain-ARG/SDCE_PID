...
// Use "Closest Point Method (CPM)" to find the closest set of position and trajectory waypoints.
int CPM(double x_position, double y_position, vector<double> x_points, vector<double> y_points) 
{
  int closest_point = 0;
  double mindist = 100000.0; // Fixed value achieved in base of empiric tests.

  for (int i = 0; i< x_points.size(); ++i) {
    double actualdist = pow(pow((x_position - x_points[i]), 2) + pow((y_position - y_points[i]), 2), 0.5);
...
int main ()
{
  ...
  /***************************************************************************************
   *                  PROCEDURE TO TUNE PID MANUALLY 
   ***************************************************************************************
  The procedure for tuning the PID loop is described below:
  - All constants are set to zero.
  - The proportional constant is increased until a response that is as close 
    to the desired response as possible is obtained. At this point it is possible 
    that over impulse or oscillations appear. The steady-state error is probably nonzero.
  - To correct the steady-state error, the integral constant must be adjusted. Overshoot 
    and oscillations may increase.
  ...
  **/
  ...
  //pid_steer.Init(0.50, 0.0, 0.0, 1.2, -1.2);
  //pid_steer.Init(0.30, 0.0, 0.0, 1.2, -1.2);
  //pid_steer.Init(0.31, 0.0, 0.0, 1.2, -1.2);
  // P + I:
  //pid_steer.Init(0.31, 0.0300, 0.0, 1.2, -1.2);
  //pid_steer.Init(0.31, 0.0100, 0.0, 1.2, -1.2);
  //pid_steer.Init(0.31, 0.0090, 0.0, 1.2, -1.2);
  //pid_steer.Init(0.31, 0.0050, 0.0, 1.2, -1.2);
  //pid_steer.Init(0.31, 0.0011, 0.0, 1.2, -1.2);
  // P + I + D:
  //pid_steer.Init(0.31, 0.0011, 0.3, 1.2, -1.2);
  //pid_steer.Init(0.31, 0.0011, 0.5, 1.2, -1.2);
  //pid_steer.Init(0.31, 0.0011, 0.68, 1.2, -1.2);
  //pid_steer.Init(0.31, 0.002, 0.2, 1.2, -1.2);
  // The best I could do :-):
  pid_steer.Init(0.31, 0.0025, 0.17, 1.2, -1.2);
  ...
  **/
  ...
  //pid_throttle.Init(0.6, 0.0, 0.0, 1.0, -1.0);
  //pid_throttle.Init(0.3, 0.0, 0.0, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.0, 0.0, 1.0, -1.0);
  // P + I:
  //pid_throttle.Init(0.21, 0.5, 0.0, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.05, 0.0, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.005, 0.0, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.0005, 0.0, 1.0, -1.0);
  // p + I + D:
  //pid_throttle.Init(0.21, 0.0005, 1.0, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.0005, 0.5, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.0005, 0.3, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.0004, 0.15, 1.0, -1.0);
  // The best I could do :-):
  pid_throttle.Init(0.21, 0.0007, 0.08, 1.0, -1.0);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          int calculated_closest_point = CPM(x_position, y_position, x_points,y_points);
          error_steer = (angle_between_points(x_position, y_position, x_points[calculated_closest_point], y_points[calculated_closest_point]) - yaw); 
          ...
          **/
          ...
          // Original value: "error_throttle = 0;"
        
          // Calculate the throttle error.
          error_throttle = (v_points[calculated_closest_point] - velocity);
          ...
          **/
          ...
          msgJson["update_point_thresh"] = 10;
